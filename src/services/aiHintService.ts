...
class AIHintService {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (isRateLimit && attempt < maxRetries - 1) {
          ...
          console.log(
            `AI Hint: Rate limited, retrying in ${delay.toFixed(0)}ms (attempt ${attempt + 1}/${maxRetries})`,
          );
  ...
  private isProperSentence(text: string): boolean {
    ...
    if (
      ...
      console.log(
        "AI Hint: ⛔ Rejecting non-sentence (metadata/instruction):",
        text,
      );
      return false;
    }
    ...
    const hasCommonWords =
      /\b(is|are|was|were|has|have|had|do|does|did|can|could|will|would|be|been|being|ik|je|hij|zij|het|wij|jullie|ben|bent|zijn|heeft|had|kan|moet|wil|gaan|komt)\b/i.test(
        lower,
      );

    if (!hasCommonWords) {
      console.log(
        "AI Hint: ⛔ Rejecting text without verb/subject structure:",
        text,
      );
      return false;
    }
    ...
    try {
      return await this.withRetry(async () => {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Search API request failed: ${response.status} ${response.statusText}`,
          );
        }

        const data: PerplexitySearchResponse = await response.json();
        console.log(
          "AI Hint: Search API returned",
        ...
        if (!data.results || data.results.length === 0) {
          console.log(
            "AI Hint: No raw results from API, trying broader search...",
          );
          ...
          const broaderData: PerplexitySearchResponse =
            await broaderResponse.json();
          console.log(
            "AI Hint: Broader search returned",
            broaderData.results?.length || 0,
            "raw results",
          );
          const broaderExamples = this.extractSentencesFromResults(
            broaderData.results || [],
            word,
          );
          console.log(
            "AI Hint: After filtering, extracted",
            broaderExamples.length,
            "A2-level examples",
          );
          return broaderExamples;
        }

        const extractedExamples = this.extractSentencesFromResults(
          data.results,
          word,
        );
        console.log(
          "AI Hint: After A2-level filtering, extracted",
  ...
  private async generateWithChat(
    ...
    return await this.withRetry(async () => {
      ...
      return sentences.length > 0
        ? sentences
        : [this.cleanMarkdownFormatting(content)];
    });
  }
  ...
  async generateHint(
    ...
    console.log(
      `AI Hint: Generating hint for word:`,
      word,
      "translation:",
      translation,
    );
    ...
    try {
      ...
      if (examples.length === 0) {
        examples = await this.generateExampleSentences(
          word,
          translation,
          targetLang,
          3,
      ...
      if (!explanation) {
        ...
        const searchedExplanation = await this.searchForExplanation(
          word,
          targetLang,
        );
  ...
  async generateExampleSentences(
    ...
    console.log(
      `AI Hint: Generating ${count} examples for word:`,
      word,
      "translation:",
      translation,
    );
    ...
    try {
      ...
      if (searchExamples.length > 0) {
        ...
        const cleanedExamples = searchExamples.slice(0, count).map((raw) => {
        ...
        console.log(
          `AI Hint: ✅ Using ${cleanedExamples.length} search results`,
        );

        // If we have fewer than requested, generate the rest
        if (cleanedExamples.length < count) {
          console.log(
            `AI Hint: Found only ${cleanedExamples.length}, generating ${count - cleanedExamples.length} more...`,
          );
          const generated = await this.generateWithChat(
            word,
            translation,
            targetLang,
            count - cleanedExamples.length,
          );
      ...
      console.log(
        "AI Hint: ❌ No A2-level examples found in search results (all too complex)",
      );
      console.log(
        `AI Hint: 🤖 Falling back to AI generation for all ${count} examples...`,
      );

      // Strategy 2: Generate with chat completions (fallback)
      const generatedSentences = await this.generateWithChat(
        word,
        translation,
        targetLang,
        count,
  ...
  async generateExampleSentence(
    ...
    const sentences = await this.generateExampleSentences(
      word,
      translation,
      targetLang,
      1,
