...
zijn,was,geweest,to be`;
  ...
  describe("checkConjugation", () => {
    ...
    it("should check infinitive form correctly", () => {
      expect(manager.checkConjugation("lopen", testVerb, "infinitive")).toBe(
        true,
      );
      expect(manager.checkConjugation("Lopen", testVerb, "infinitive")).toBe(
        true,
      );
      expect(manager.checkConjugation("wrong", testVerb, "infinitive")).toBe(
        false,
      );
    });

    it("should check past_singular form correctly", () => {
      expect(manager.checkConjugation("liep", testVerb, "past_singular")).toBe(
        true,
      );
      expect(manager.checkConjugation("LIEP", testVerb, "past_singular")).toBe(
        true,
      );
      expect(manager.checkConjugation("wrong", testVerb, "past_singular")).toBe(
        false,
      );
    });
    ...
    it("should check past_participle form correctly", () => {
      expect(
        manager.checkConjugation("gelopen", testVerb, "past_participle"),
      ).toBe(true);
      expect(
        manager.checkConjugation("  gelopen  ", testVerb, "past_participle"),
      ).toBe(true);
      expect(
        manager.checkConjugation("wrong", testVerb, "past_participle"),
      ).toBe(false);
    });
    ...
    it("should handle multiple correct forms separated by slash", () => {
      ...
      expect(
        manager.checkConjugation("liep", verbWithMultiple, "past_singular"),
      ).toBe(true);
      expect(
        manager.checkConjugation("liepen", verbWithMultiple, "past_singular"),
      ).toBe(true);
    });
  });
